...
public class KeyVitProcedure {
	public static void execute(Entity entity) {
		...
		if ((entity.getCapability(EpicStatsModRemasteredModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new EpicStatsModRemasteredModVariables.PlayerVariables())).SP > 0) {
			{
				double _setval = (entity.getCapability(EpicStatsModRemasteredModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new EpicStatsModRemasteredModVariables.PlayerVariables())).stat_2_level + 1;
			...
			if ((double) ExpManagerConfiguration.EF_WEIGHT_STAT.get() == 2) {
				{
			...
			VitSetProcedure.execute(entity);
		}
	}
}
